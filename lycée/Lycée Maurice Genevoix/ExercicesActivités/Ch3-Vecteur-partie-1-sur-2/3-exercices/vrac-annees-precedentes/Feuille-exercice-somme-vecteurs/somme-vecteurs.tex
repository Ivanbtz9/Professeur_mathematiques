...
\indent\vspace{-3cm}
$$\fbox{\text{\begin{Large}Vecteurs - Feuille d'exercices sur la somme de deux vecteurs \end{Large}}}$$
...
Dans chacun des cas suivants, construire les points $M$ et $N$ définis par les égalités vectorielles données :\\
\\
...
Construire à l'aide du compas, dans chacun des cas suivants, le vecteur $\V{u}+\V{v}$.\\
\\
...
\noindent\underline{\textbf{Exercice 4 : }}\\  % Odyssée 2014 p.180 exo 132
On considère les vecteurs représentés dans le quadrillage ci-dessous :
$$\includegraphics[scale=1]{exo-4-guitare.eps}$$
\begin{enumerate}[$\diamond$]
\item 
		\begin{enumerate}[$a.$]
		\item Placer un point $A$ sur la gauche d'une feuille quadrillée. 
...
\item
		...
		\item Cpnstruire le point $J$ tel que :
		$$\V{IJ}==\V{PV}+0,5\V{PS}$$
		...
		$$\V{MQ}=\V{PT}-\V{PO}$$
		\end{enumerate}
\item Tracer la ligne brisée $ABCDFGHIJKLMQ$.
\item Finir la construction par symétrie de la ligne brisée par rapport à la droite $(AQ)$.
\end{enumerate}
